...
Public Class GlobalPlayerRecordsForm
    ...
    Private Sub GlobalPlayerRecordsForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.UserRecordsTableAdapter.Fill(Me.DivRPUserRecordsDataSet.UserRecords)
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        ...
        Else
            ...
            Else
                ...
                myConnection.Open()
                Dim str As String
                str = "DELETE FROM UserRecords WHERE ID=" & DgvGlobalUserRecords.Item("ID", i).Value
                Dim cmd As SqlCommand = New SqlCommand(str, myConnection)

                Try
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                    UserRecordsTableAdapter.Fill(DivRPUserRecordsDataSet.UserRecords)
                    Me.UserRecordsTableAdapter.Fill(Me.DivRPUserRecordsDataSet.UserRecords)
                    If IsNothing(DgvGlobalUserRecords.CurrentRow) = False Then
                        Me.DgvGlobalUserRecords.CurrentRow.Selected = True
                    End If
